...
function EditComponent(props) {
  ...
  (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
    if (props.attributes.profId) {
      //don't provide preview if no professor selected
      updateTheMeta();
      async function go() {
  ...
  function updateTheMeta() {
    ...
    const profsForMeta = wp.data.select("core/block-editor").getBlocks() //every block on post/page
    .filter(block => block.name === "ourplugin/featured-professor").map(block => block.attributes.profId).filter((ID, index, arr) => {
