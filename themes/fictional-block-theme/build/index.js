...
class Like {
  constructor() {
    if (document.querySelector(".like-box")) {
      (axios__WEBPACK_IMPORTED_MODULE_0___default().defaults).headers.common["X-WP-Nonce"] = MYSCRIPT.nonce;
      this.events();
    }
  }
  events() {
  ...
  async createLike(currentLikeBox) {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(MYSCRIPT.site_url + "/wp-json/university/v1/manageLike", {
...
class MyNotes {
  ...
  deleteNote(e) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      type: 'DELETE',
      success: response => {
        if (response.userNoteCount < 5) jquery__WEBPACK_IMPORTED_MODULE_0___default()('.note-limit-message').removeClass("active");
      },
      error: response => {
...
class Search {
  ...
  getResults() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().getJSON(MYSCRIPT.site_url + '/wp-json/university/v1/search?term=' + this.searchField.val(), results => {
